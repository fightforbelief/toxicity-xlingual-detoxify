...
\usepackage{titling}
\setlength{\droptitle}{-6em}   % pull title upward
\title{\vspace{-1.5em}
Multilingual Toxic Comment Detection:\\
Translated vs.\ Original-Language Training with \textit{Detoxify}\\[-3em]
}
\author{
CIS 5300 M2 \quad
\textit{Zixuan Bian \quad Aria Shi \quad Siyuan Shen \quad Alex Yang} \\
November 20, 2025 \\[-0.3em]
}
\date{}
\begin{document}
\maketitle
...
Given a threshold $\tau$, we define the binary prediction $ \hat{y}_i = \mathbf{1}[\hat{p}_i \ge \tau]$.
From the resulting confusion matrix we obtain $ TP, FP, TN, FN \in \mathbb{N}$, the usual numbers of true positives, false positives, true negatives, and false negatives.
...
Our simple baseline is a traditional TF--IDF + Logistic Regression classifier trained separately for each language. We lowercase the text, remove URLs and mentions, and represent each comment with word-level TF--IDF features (1--2 grams, up to 200k dimensions). We then train a Logistic Regression model with balanced class weights and \texttt{max\_iter=2000}, select the decision threshold that maximizes F1 on the validation set, and evaluate on the test set.
...
Our strong baseline is the multilingual Detoxify model~\citep{detoxify}, which uses an XLM--RoBERTa encoder~\citep{xlmr} to produce toxicity probabilities. We load the pretrained \texttt{Detoxify("multilingual")} checkpoint and apply it to our validation and test splits in each language. For each language, we obtain a score $\hat{p}_i$ for every comment, select a decision threshold on the validation set to maximize F1, and then evaluate ROC--AUC and F1 on the test set.
